...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .note { margin: 0; color: var(--muted); font-size: 0.9rem; max-width: 72rem; }
    .controls {
      display: flex;
      flex-wrap: wrap;
      gap: 12px 18px;
    ...
    .status { color: var(--muted); font-size: 0.85rem; padding: 0 20px 12px; }
...
<body>
  <header>
    ...
    <p class="note">
      ...
      Default metric is production density (kg dry matter per total cell hectare), which stays readable where suitability is sparse.
      Yield and suitable fraction are available as secondary views.
    </p>
  </header>
  <div class="controls">
    <label>Crop
  ...
  function percentile(values, p) {
    ...
    for (let i = 0; i < values.length; i++) {
      const v = values[i];
      if (Number.isFinite(v) && v > 0) filtered.push(v);
    }
  ...
  async function main() {
    ...
    for (const metric of meta.metrics) {
      ...
      if (metric.id === meta.default_metric) opt.selected = true;
      metricSelect.appendChild(opt);
    }

    status.textContent = "Loading location id map…";
    const idBytes = await loadGzip(meta.assets.location_id_map);
    const idMap = new Uint16Array(idBytes.buffer);
    const width = meta.map.width;
    const height = meta.map.height;
    if (idMap.length !== width * height) {
    ...
    const attributes = new Float32Array(attrBytes.buffer);
    ...
    function valueAt(rowIndex1Based, colIdx) {
      ...
      const row = rowIndex1Based - 1;
      return attributes[colIdx * n + row];
    }
    ...
    function recolor() {
      ...
      const slice = attributes.subarray(colIdx * n, (colIdx + 1) * n);
      const vmax = Math.max(percentile(slice, 0.98), 1e-6);
      ...
      document.getElementById("legendMax").textContent = formatValue(vmax, metricId);
      ...
      status.textContent = `${meta.crops.find(c => c.id === crop).label} · ${water} · ${metric.label}`;
    }
    ...
    canvas.addEventListener("mousemove", (evt) => {
      ...
      const value = valueAt(id, colIdx);
      tooltip.style.display = "block";
      tooltip.style.left = (evt.offsetX + 14) + "px";
      tooltip.style.top = (evt.offsetY + 14) + "px";
      tooltip.innerHTML = `<strong>${tag}</strong><br>${metric.label}: ${formatValue(value, metricId)} ${metric.unit}<br>${water}`;
    });
    canvas.addEventListener("mouseleave", () => { tooltip.style.display = "none"; });

    cropSelect.addEventListener("change", recolor);
    waterSelect.addEventListener("change", recolor);
    metricSelect.addEventListener("change", recolor);
    recolor();
  }
...
def publish_docs(
    *,
    ...
    (docs_dir / "index.html").write_text(VIEWER_HTML, encoding="utf-8")
